defmodule PokequizWeb.Games.TypeCombinationLive.Show do
  ...
  def handle_event("new", _, %{assigns: %{quiz: quiz, name: name}} = socket) do
    types = Dex.Type.random_kombo_with_pokemon(socket.assigns.settings.generations)
  ...
  def init(socket) do
    types = Dex.Type.random_kombo_with_pokemon(socket.assigns.game.settings.generations)
