defmodule PokequizWeb.Games.WhoisLive.Show do
  ...
  def render(assigns) do
    ...
    <div>
      <div class="relative flex justify-center">
        ...
        />
        <img
          class={[@quiz.pick, "2xl:w-4/12"]}
          width="40%"
          height="40%"
          alt="Unkown Pokemon"
          src={Dex.Pokemon.image(@quiz.pokemon)}
        />

        <form
          :if={@player && !@quiz.finished}
          class="absolute bottom-4 left-56 text-black font-bold text-xl p-2 rounded flex justify-center gap-2"
          phx-submit="submit"
        ...
        <p
          :if={@quiz.finished}
          class="absolute origin-center -rotate-12 text-4xl font-bold text-yellow-400 right-[20%] top-[35%]"
        >
          {@quiz.pokemon.name}
        </p>
      </div>
  ...
  def handle_event("submit", %{"input_value" => msg}, socket) do
    ...
    if quiz.finished do
      ...
      finished =
        String.downcase(pokemon.name) == String.downcase(msg) or
          String.downcase(Enum.at(pokemon.species.names, 5).name) == String.downcase(msg)
